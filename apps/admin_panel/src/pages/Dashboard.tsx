...
export default function Dashboard() {
  ...
  const avgCompliance = policies.length > 0 ? Math.round(policies.reduce((acc, p) => acc + p.complianceRate, 0) / policies.length) : 0

  // Combine current active user with registered users list
  const activeUsers = user ? [user, ...registeredUsers.map(r => r.profile).filter(p => p.email !== user.email)] : registeredUsers.map(r => r.profile)
    ...
    <div style={{ padding: '32px 36px', maxWidth: 1400, display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 14, fontWeight: 500 }}>
          Welcome back, <strong style={{ color: 'var(--accent)' }}>@{user?.username || 'Admin'}</strong> — corporate position: <strong style={{ color: 'var(--accent)' }}>{user?.role || 'Workspace Admin'}</strong>.
        </p>
      </div>

      {/* Primary KPI Stats Grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20 }}>
        <StatCard title="Active Teams"      value={`${teams.length} Teams`} change="0%" positive icon="🏢" color="#336659" />
        <StatCard title="Assigned Tasks"    value={`${tasks.length} Active`} change="0%" positive icon="📋" color="#3b82f6" />
        <StatCard title="Policy Sign-offs"  value={`${avgCompliance}% Compliance`} change="0%" positive icon="🛡️" color="#10b981" />
        <StatCard title="Open Reports"      value="0" change="0%" positive icon="🚩" color="#b34a4a" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 32 }}>
        ...
        <div className="nm-card" style={{ padding: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>{activeUsers.length} Total Users</span>
          </div>
              ...
              activeUsers.map((u, i) => (
                <div key={i} className="nm-card-inset" style={{
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    ...
                    }}>{u.avatar || u.username[0]?.toUpperCase() || '👤'}</div>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>@{u.username}</p>
                      <p style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>{u.role} ({u.category})</p>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6, fontWeight: 500 }}>{u.email}</p>
                  </div>
                </div>
              ))
            ) : (
              <div className="nm-card-inset" style={{ padding: '24px', textAlign: 'center', color: 'var(--text-muted)', borderRadius: '14px' }}>
                <p style={{ fontSize: '14px', fontWeight: 600 }}>No user signups yet</p>
                <p style={{ fontSize: '12px', marginTop: '4px' }}>Registered employees will appear here.</p>
              </div>
            )}
          </div>
        </div>
